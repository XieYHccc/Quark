...
glm::mat3 corss_matrix(const glm::vec3& vec) {
	glm::mat3 mat(0.f);
	mat[0][1] = vec[2];
	mat[0][2] = -vec[1];
	mat[1][0] = -vec[2];
...
bool check_collision(PlaneCollider& plane, MeshCollider& mesh) {
	...
	// plane attributes
	glm::vec3 plane_normal = plane.normal_;
	glm::vec3 plane_pos = plane.position_;
	// mesh attributes
	...
	int sum = 0;
	for (const auto& pos : mesh.trimesh_->get_positions()) {
	...
	if (mesh_rigid_body != nullptr) {
		...
		float uN = glm::length(vi) < 1 ? 0.1f : 0.5f; // bump coefficient
		float uT = 0.5; // friction coefficient
		...
		// 3.calculate impulse
		assert(mass > 0);
		glm::mat3 cross_mat = corss_matrix(collision_pos);
