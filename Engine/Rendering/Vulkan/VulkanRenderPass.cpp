    ...
    u32 stencil)
{
    ...
    out_renderpass->region = region;
    out_renderpass->clearColor = ckearColor;
    ...
    color_attachment.format = driver->swapChain->surfaceFormat.format; // TODO: configurable
    ...
    depth_attachment.format = driver->device->GetDepthFormat();
...
void VulkanRenderPassFree(RenderDeviceDriver_Vulkan* driver, RenderPass_Vulkan* renderpass)
{
    if (renderpass && renderpass->handle) {
        vkDestroyRenderPass(driver->device->logicalDevice, renderpass->handle, nullptr);
    ...
    VkFramebuffer frame_buffer)
{
    ...
    begin_info.renderArea.offset.x = renderpass->region.x;
    begin_info.renderArea.offset.y = renderpass->region.y;
    begin_info.renderArea.extent.width = renderpass->region.z;
    begin_info.renderArea.extent.height = renderpass->region.w;

    VkClearValue clear_values[2];
    memset(clear_values, 0,  sizeof(VkClearValue) * 2);
    clear_values[0].color.float32[0] = renderpass->clearColor.r;
    clear_values[0].color.float32[1] = renderpass->clearColor.g;
    clear_values[0].color.float32[2] = renderpass->clearColor.b;
    clear_values[0].color.float32[3] = renderpass->clearColor.a;
