...
std::shared_ptr<Mesh> Mesh::AddToPool(const MeshCreateInfo &info)

{
    ...
    assetPool_[uuid] = std::make_shared<Mesh>(info);
...
Mesh::Mesh(const MeshCreateInfo& info)
    : BaseAsset(info.path, info.path)
{

	const size_t vertexBufferSize = info.vertices.size() * sizeof(Vertex);
	const size_t indexBufferSize = info.indices.size() * sizeof(uint32_t);
	...
	// copy vertex buffer
	memcpy(data, info.vertices.data(), vertexBufferSize);
	// copy index buffer
	memcpy((char*)data + vertexBufferSize, info.indices.data(), indexBufferSize);
    ...
    // Set submeshes
    submeshs_ = info.submeshs;
	dynamic_ = info.dynamic;
}
