...
{
	m_camera_params.projection = proj;
	m_camera_params.view = view;
	m_camera_params.view_projection = proj * view;
	...
	glm::mat2 inv_zw(m_camera_params.inv_projection[2].zw(), m_camera_params.inv_projection[3].zw());
	const auto project = [](const glm::vec2& zw) -> float {
		return -zw.x / zw.y;
	};
	m_camera_params.z_near = project(inv_zw * glm::vec2(0.0f, 1.0f));
	m_camera_params.z_far = project(inv_zw * glm::vec2(1.0f, 1.0f));
	
}
void RenderContext::SetLightingParameters(const LightingParameters* lighting)
{
	m_lighting_params = lighting;
}
