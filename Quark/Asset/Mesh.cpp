...
void Mesh::UpdateGpuBuffers()
{
    ...
    if (!m_VertexBuffer)
    {
        ...
        vert_buffer_desc.size = m_OverlappedVertexData.size();
        vert_buffer_desc.usageBits = graphic::BUFFER_USAGE_VERTEX_BUFFER_BIT | graphic::BUFFER_USAGE_TRANSFER_TO_BIT;
        m_VertexBuffer = graphic_device->CreateBuffer(vert_buffer_desc, m_OverlappedVertexData.data());
    }
    else
    {   // Only dynamic mesh can be updated
        CORE_DEBUG_ASSERT(isDynamic && m_VertexBuffer->GetDesc().domain == graphic::BufferMemoryDomain::CPU)

        void* mappedData = m_VertexBuffer->GetMappedDataPtr();
        memcpy(mappedData, m_OverlappedVertexData.data(), m_OverlappedVertexData.size());
    }
...
void Mesh::ReCalculateAabb()
{
    for (auto& submesh : this->subMeshes)
    {
        ...
        for (size_t i = 0; i < submesh.count; i++) 
        {
            glm::vec3 position = positions[indices[submesh.startIndex + i]];
