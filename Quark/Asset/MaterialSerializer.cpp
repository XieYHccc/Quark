...
std::string MaterialSerializer::SerializeToYaml(const Ref<Material>& materialAsset)
{
    ...
    out << YAML::Key << "Material" << YAML::Value;
    {
        ...
        // Textures
        if (materialAsset->baseColorTexture->image != RenderSystem::Get().GetRenderResourceManager().image_white)
        ...
        if ( materialAsset->metallicRoughnessTexture && materialAsset->metallicRoughnessTexture->image != RenderSystem::Get().GetRenderResourceManager().image_white)
...
bool MaterialSerializer::DeserializeFromYaml(const std::string& yamlString, Ref<Material>& outMaterial)
{
    ...
    defaultTexture->image = RenderSystem::Get().GetRenderResourceManager().image_white;
    defaultTexture->sampler = RenderSystem::Get().GetRenderResourceManager().sampler_linear;
    ...
    outMaterial->shaderProgram = RenderSystem::Get().GetRenderResourceManager().GetShaderLibrary().program_staticMesh;
